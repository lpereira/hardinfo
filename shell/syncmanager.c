...
static gboolean send_request_for_net_action(SyncNetAction *sna)
{
 ...
 if (!sna->entry->optional || (our_blobs_update_version < server_blobs_update_version))
 {
  if (strncmp(sna->entry->file_name, "blobs-update-version.json", 25) == 0)
  {
   uri = g_strdup_printf("%s/%s?ver=%s&blobver=%d&rel=%d", API_SERVER_URI, sna->entry->file_name, VERSION,
                         our_blobs_update_version, RELEASE);
  }
  ...
  { // Blocking/Sync sending when no gui
...
static SyncDialog *sync_dialog_new(GtkWidget *parent)
{
 ...
 gtk_window_set_icon(GTK_WINDOW(dialog), icon_cache_get_pixbuf(SVG_SYNC));
 ...
 gtk_box_pack_start(GTK_BOX(hbox), icon_cache_get_image_at_size(SVG_SYNC, 64, 64), FALSE, FALSE, 0);
  ...
  else
  {
   ...
   if (((strncmp(action->entry->file_name, "benchmark.json", 14) == 0) &&
       ...
       ((!(strncmp(action->entry->file_name, "benchmark.json", 14) == 0) ||
         (!action->entry->generate_contents_for_upload)) &&
        (!send_benchmark)))
   {
    loop = g_main_loop_ref(loop);
    sync_one(action);
