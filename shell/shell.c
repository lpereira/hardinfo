...
static void stylechange3_me(void)
{
 ...
 int newDark = 0, i = 0;
 gchar **keys = NULL;
 if (settings && !newgnome)
  keys = g_settings_list_keys(settings);
 while (!newgnome && keys && (keys[i] != NULL))
 {
  if (strcmp(keys[i], "color-scheme") == 0)
   newgnome = 1;
  g_free(keys[i]);
  i++;
 }
...
static ShellNote *note_new(void)
{
 ...
 icon = icon_cache_get_image(SVG_QUIT);
...
#endif
 icon = icon_cache_get_image_at_size(SVG_DIALOG_INFO, 32, 32);
...
static void create_window(void)
{
 ...
 gtk_window_set_icon(GTK_WINDOW(shell->window), icon_cache_get_pixbuf(SVG_ABOUT));
...
#if GTK_CHECK_VERSION(3, 0, 0)
 if (params.theme == -1)
  shell_action_set_active("DisableThemeAction", TRUE);
 if (params.theme == 1)
  shell_action_set_active("Theme1Action", TRUE);
 if (params.theme == 2)
  shell_action_set_active("Theme2Action", TRUE);
 if (params.theme == 3)
  shell_action_set_active("Theme3Action", TRUE);
 if (params.theme == 4)
  shell_action_set_active("Theme4Action", TRUE);
 if (params.theme == 5)
  shell_action_set_active("Theme5Action", TRUE);
 if (params.theme == 6)
  shell_action_set_active("Theme6Action", TRUE);
#endif
...
void shell_add_modules_to_gui(gpointer _shell_module, gpointer _shell_tree)
{
 ...
 if (module->entries)
 {
  ...
  for (p = module->entries; p; p = g_slist_next(p))
  {
   ...
   if (entry->flags & MODULE_FLAG_HIDE)
    continue;
...
#define RANGE_SET_VALUE(tree, scrollbar, value)                                                                         \
 {                                                                                                                      \
  while (gtk_events_pending())                                                                                          \
   gtk_main_iteration();                                                                                                \
  gtk_range_set_value(GTK_RANGE(gtk_scrolled_window_get_##scrollbar(GTK_SCROLLED_WINDOW(shell->tree->scroll))), value); \
 }
#define RANGE_GET_VALUE(tree, scrollbar) \
 gtk_range_get_value(GTK_RANGE(gtk_scrolled_window_get_##scrollbar(GTK_SCROLLED_WINDOW(shell->tree->scroll))))
#else
#define RANGE_SET_VALUE(tree, scrollbar, value)                       \
 do                                                                   \
 {                                                                    \
...
static void update_progress()
{
 ...
 do
 {
  ...
  gtk_tree_store_set(store, &iter, INFO_TREE_COL_PROGRESS, pct,
                     INFO_TREE_COL_VALUE, strreplacechr(formatted, ",", '.'), -1);
                                           ...
                                           gsize ngroups)
{
 ...
 if (params.theme > 0)
 {
  if (params.darkmode)
  {
   gtk_css_provider_load_from_data(provider, "treeview { background-color: rgba(0xa0, 0xa0, 0xa0, 0.1); } "
                                             "treeview:selected { background-color: rgba(0x60, 0x80, 0xff, 1); } ",
                                   -1, NULL);
   gtk_style_context_add_provider(gtk_widget_get_style_context(shell->info_tree->view), GTK_STYLE_PROVIDER(provider),
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  }
  else
  {
   gtk_css_provider_load_from_data(provider, "treeview { background-color: rgba(0x60, 0x60, 0x60, 0.1); } "
                                             "treeview:selected { background-color: rgba(0x40, 0x60, 0xff, 1); } ",
                                   -1, NULL);
   gtk_style_context_add_provider(gtk_widget_get_style_context(shell->info_tree->view), GTK_STYLE_PROVIDER(provider),
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  }
...
static gchar *vendor_info_markup(const Vendor *v)
{
 ...
 if (v->url_support)
 {
  ...
  ven_mt = appf(ven_mt, "\n", "<b>%s:</b> <a href=\"%s\">%s</a>", _("Support URL"),
                full_link ? full_link : v->url_support, v->url_support);
  g_free(full_link);
  full_link = NULL;
 }
 if (v->wikipedia)
 {
  ...
  ven_mt = appf(ven_mt, "\n", "<b>%s:</b> <a href=\"%s\">%s</a>", _("Wikipedia"),
                full_link ? full_link : v->wikipedia, v->wikipedia);
  g_free(full_link);
  full_link = NULL;
 }
 g_free(ven_tag);
...
static gchar *detail_view_clear_value(gchar *value)
{
 ...
 else if (g_key_file_load_from_data(keyfile, value,
                                    strlen(value), 0, NULL))
 {
  gchar **groups;
  gint group;
...
static ShellTree *tree_new()
{
 ...
 if (params.theme > 0)
 {
  gtk_css_provider_load_from_data(provider, "treeview { background-color: rgba(0x60, 0x60, 0x60, 0.1); } "
                                            "treeview:selected { background-color: rgba(0x40, 0x60, 0xff, 1); } ",
                                  -1, NULL);
  gtk_style_context_add_provider(gtk_widget_get_style_context(treeview), GTK_STYLE_PROVIDER(provider),
                                 GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
 }
#endif
