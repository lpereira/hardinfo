...
void cb_theme1()
{
 if (shell_action_get_active("Theme1Action"))
 {
  ...
  if (shell_action_get_active("DisableThemeAction"))
   shell_action_set_active("DisableThemeAction", FALSE);
  if (shell_action_get_active("Theme2Action"))
   shell_action_set_active("Theme2Action", FALSE);
  if (shell_action_get_active("Theme3Action"))
   shell_action_set_active("Theme3Action", FALSE);
  if (shell_action_get_active("Theme4Action"))
   shell_action_set_active("Theme4Action", FALSE);
  if (shell_action_get_active("Theme5Action"))
   shell_action_set_active("Theme5Action", FALSE);
  if (shell_action_get_active("Theme6Action"))
   shell_action_set_active("Theme6Action", FALSE);
 }
 cb_disable_theme();
}
void cb_theme2()
{
 if (shell_action_get_active("Theme2Action"))
 {
  ...
  if (shell_action_get_active("DisableThemeAction"))
   shell_action_set_active("DisableThemeAction", FALSE);
  if (shell_action_get_active("Theme1Action"))
   shell_action_set_active("Theme1Action", FALSE);
  if (shell_action_get_active("Theme3Action"))
   shell_action_set_active("Theme3Action", FALSE);
  if (shell_action_get_active("Theme4Action"))
   shell_action_set_active("Theme4Action", FALSE);
...
void cb_disable_theme()
{
 ...
 gtk_css_provider_load_from_data(providera,
                                 "notebook {background-color:unset;} scrolledwindow {background-color:unset;} viewport "
                                 "{background-color:unset;} frame {background-color:unset;}",
                                 -1, NULL);
 gtk_style_context_add_provider_for_screen(gdk_screen_get_default(), GTK_STYLE_PROVIDER(providera),
 ...
 { // enable
  if (params.darkmode)
  {
   sprintf(theme_st, "window.background {background-image: url(\"%s/pixmaps/bg%d_dark.jpg\"); background-repeat: "
                     "no-repeat; background-size:100%% 100%%; }",
           params.path_data, params.theme);
  }
  else
  {
   sprintf(theme_st, "window.background {background-image: url(\"%s/pixmaps/bg%d_light.jpg\"); background-repeat: "
                     "no-repeat; background-size:100%% 100%%; }",
           params.path_data, params.theme);
  }
  gtk_css_provider_load_from_data(provider, theme_st, -1, NULL);
  if (shell->window)
   gtk_style_context_add_provider(gtk_widget_get_style_context(shell->window), GTK_STYLE_PROVIDER(provider),
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  if (params.darkmode)
  {
   // menubar
   gtk_css_provider_load_from_data(provider2, "* { background-color: rgba(0xa0, 0xa0, 0xa0, 0.1); } * "
                                              "text { background-color: rgba(1, 1, 1, 1); }",
                                   -1, NULL);
   if (shell->ui_manager)
    gtk_style_context_add_provider(gtk_widget_get_style_context(gtk_ui_manager_get_widget(shell->ui_manager,
                                                                                          "/MainMenuBarAction")),
                                   GTK_STYLE_PROVIDER(provider2), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
   // treeviewinfo
   gtk_css_provider_load_from_data(provider3, "treeview { background-color: rgba(0xa0, 0xa0, 0xa0, 0.1); } "
                                              "treeview:selected { background-color: rgba(0x60, 0x80, 0xff, 1); } ",
                                   -1, NULL);
   if (shell->info_tree && shell->info_tree->view)
    gtk_style_context_add_provider(gtk_widget_get_style_context(shell->info_tree->view), GTK_STYLE_PROVIDER(provider3),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
   // treeviewmain
   gtk_css_provider_load_from_data(provider3, "treeview { background-color: rgba(0xa0, 0xa0, 0xa0, 0.1); } "
                                              "treeview:selected { background-color: rgba(0x60, 0x80, 0xff, 1); } ",
                                   -1, NULL);
   if (shell->tree && shell->tree->view)
    gtk_style_context_add_provider(gtk_widget_get_style_context(shell->tree->view), GTK_STYLE_PROVIDER(provider3),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  }
  else
  {
   // menubar
   gtk_css_provider_load_from_data(provider2, "* { background-color: rgba(0x60, 0x60, 0x60, 0.1); } * "
                                              "text { background-color: rgba(1, 1, 1, 1); }",
                                   -1, NULL);
   if (shell->ui_manager)
    gtk_style_context_add_provider(gtk_widget_get_style_context(gtk_ui_manager_get_widget(shell->ui_manager,
                                   ...
                                   GTK_STYLE_PROVIDER(provider2), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
   // treeviewinfo
   gtk_css_provider_load_from_data(provider3, "treeview { background-color: rgba(0x60, 0x60, 0x60, 0.1); } "
                                              "treeview:selected { background-color: rgba(0x30, 0x50, 0xff, 1); } ",
                                   -1, NULL);
   if (shell->info_tree && shell->info_tree->view)
    gtk_style_context_add_provider(gtk_widget_get_style_context(shell->info_tree->view), GTK_STYLE_PROVIDER(provider3),
                                   GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
...
void cb_about()
{
  ...
  if (st.st_size > 0)
  {
   ...
   if (buf)
   {
    ...
    if (i > 0)
    {
     ...
     if (s && sscanf(s, "\"latest-program-version\":\"%u.%u.%u\"", &u1, &u2, &u3) == 3)
      latest_ver = u1 * 10000 + u2 * 100 + u3;
    }
   }
  }
  close(fd);
 ...
 free(path);
 if (sscanf(VERSION, "%u.%u.%u", &a1, &a2, &a3) == 3)
  app_ver = a1 * 10000 + a2 * 100 + a3;
 ...
 gtk_about_dialog_set_logo(GTK_ABOUT_DIALOG(about), icon_cache_get_pixbuf_at_size(SVG_ABOUT, 128, 128));
