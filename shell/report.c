...
static void report_html_header(ReportContext *ctx)
{
 ...
 ctx->output =
     g_strdup_printf("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Final//EN\">\n"
                     "<html><head>\n"
...
report_html_key_value(ReportContext *ctx, gchar *key, gchar *value, gsize longest_key)
{
 ...
 else
 {
  ...
  ctx->output = h_strdup_cprintf("\n<tr%s>\n<td class=\"icon\">%s</td><td class=\"field\">%s</td>", ctx->output,
                                 highlight ? " class=\"hilight\"" : "", icon, name);
...
static ReportDialog *report_dialog_new(GtkTreeModel *model, GtkWidget *parent)
{
 ...
 gtk_window_set_icon(GTK_WINDOW(dialog), icon_cache_get_pixbuf(SVG_REPORT));
 ...
 gtk_box_pack_start(GTK_BOX(hbox), icon_cache_get_image_at_size(SVG_REPORT, 64, 64), FALSE, FALSE, 0);
