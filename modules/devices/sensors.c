...
static void read_sensors_windfarm(void) {
    ...
    if (g_file_test(path_wf, G_FILE_TEST_EXISTS)) {
        GDir *wf;

        if ((wf = g_dir_open(path_wf, 0, NULL))) {
            GRegex *regex;
            GError *err = NULL;
            const gchar *entry;
                    ...
                    if (g_regex_match(regex, entry, 0, NULL)) {
                        ...
                        if (g_file_get_contents(path, &tmp, NULL, NULL)) {
                            ...
                            if (sensor->with_decimal_p) {
                                // format source https://elixir.free-electrons.com/linux/v5.14/source/drivers/macintosh/windfarm_core.c#L301
