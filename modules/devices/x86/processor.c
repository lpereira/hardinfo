...
{
 gint i = 0;
 i = g_strcmp0(a->shared_list, b->shared_list);
 if (i != 0)
  return i;
...
#define NULLIFNOTYES(f)                 \
 if (processor->f)                      \
  if (strcmp(processor->f, "yes") != 0) \
  {                                     \
   g_free(processor->f);                \
   processor->f = NULL;                 \
  }
...
gchar *processor_get_info(GSList *processors)
{
 ...
 for (l = processors; l; l = l->next)
 {
  ...
  // icons = h_strdup_cprintf("Icon$CPU%d$cpu%d=" SVG_PROCESSOR "\n", icons, processor->id, processor->id);
 ...
 ret = g_strdup_printf("[$ShellParam$]\n"
                       ...
                       "[Processors]\n"
                       "%s",
                       _("Device"), _("Frequency"), _("Model"), _("Socket:Core"), icons, tmp);
 g_free(tmp);
 g_free(icons);
