...
void hi_show_pci_info(MainWindow *mainwindow, PCIDevice *device)
{
	...
	if(device->io_addr) {
		buf = g_strdup_printf("I/O address: 0x%x to 0x%x", device->io_addr,
			device->io_addr_end);
	...
	if(device->memory) {
		buf = g_strdup_printf("Memory: %ld KB", device->memory);
	...
	if(device->freq) {
		buf = g_strdup_printf("Frequency: %dMHz", device->freq);
	...
	if(device->latency) {
		buf = g_strdup_printf("Latency: %d", device->latency);
	...
	if(device->bus_master) {
		label = gtk_label_new("Bus master");
		gtk_widget_show(label);
		gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);
	}

	buf = g_strdup_printf("Bus: %d, Device: %d, Function: %d",
...
PCIDevice *hi_scan_pci(void)
{
	...
	while(fgets(buffer, 256, lspci)){
		...
		if(!strncmp(buf, "Flags", 5)){
			...
			factor = (unit == 'K') ? 1 : 1024;
	
			pci_dev->memory+=mem * factor;
