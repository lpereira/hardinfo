...
ParportDevice *hi_scan_parport(void)
{
	...
	for (i = 0; i <= 16; i++) {
		...
		if(!parport_dev->modes)
			parport_dev->modes = g_strdup("N/A");
...
void hi_show_parport_info(MainWindow *mainwindow, ParportDevice *device)
{
	...
	gtk_frame_set_label(GTK_FRAME(mainwindow->frame), "Parallel Port");
...
//	}
	...
	if (device->description) {
		buf = g_strdup_printf("Description: %s", device->description);
		label = gtk_label_new(buf);
		gtk_widget_show(label);
		gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);
	...
	if (device->cmdset) {
		buf = g_strdup_printf("Command set: %s", device->cmdset);
		label = gtk_label_new(buf);
		gtk_widget_show(label);
		gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);
	...
	buf = g_strdup_printf("Class: %s", type2icon[i].label);
	label = gtk_label_new(buf);
	gtk_widget_show(label);
	gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);	
	g_free(buf);	

	buf = g_strdup_printf("Base I/O address: 0x%x", device->port);
	label = gtk_label_new(buf);
	gtk_widget_show(label);
	gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);	
	g_free(buf);	

	buf = g_strdup_printf("Modes: %s", device->modes);
	label = gtk_label_new(buf);
	gtk_widget_show(label);
	gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);	
	g_free(buf);	
